...
void setup () {
  ...
  // --- early serial init: bring up the UART BEFORE any I2C so a stuck bus
  //     (e.g. a miswired sensor holding SDA/SCL low) can't blank the monitor ---
  Serial.begin(500000);                          // 500000 baud = max for 8MHz
  Serial.println(F("\nBooting... initialising I2C / RTC"));  Serial.flush();
...
//------------------------------------------------------------------------------------------------------------
  ...
  Wire.setWireTimeout(25000, true);                 // 25ms TWI timeout + auto bus-reset: a stuck I2C line fails fast instead of hanging setup() forever
...
//===============================================================================================
  ...
  // Serial.begin(500000) already called at the top of setup() - moved there so a stuck I2C bus can't blank the monitor
...
//------------------------------------------------------------------------------------------------------------
  ...
  Serial.setTimeout(1500);                                  // short window: readString() returns ~1.5s after you type, so no line-ending is needed
  unsigned long startMillis = millis();

do { command = Serial.readString();                  // read serial monitor data into the string until carridge return = \n character
    command.trim();                                              // BULLETPROOF: strip trailing CR/whitespace so 'start' matches under any serial line-ending setting
    ...
    } else if(command.indexOf("test") >= 0)                // 'test' is a HIDDEN option not on the displayed menu
