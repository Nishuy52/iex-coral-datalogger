...
3. **Attach the deployment sensors before Jun 22**, even though INSTRUCTIONS.md sequences
   them after burn-in. Sensor soldering after the deadline is not an option, so Stage 7
...
| **Fri 12 Jun** | Bootloader-check ≥4 ProMinis with `01_UploadBlink`, keep the best 3. Stage 1 mods (regulator, LED resistor, reset switch, UART header) on U01–U03. Re-verify blink after mods. | Stage 1 |
| **Sat 13 Jun** | Stage 2 RTC module mods ×3. Stage 3: join modules, solder I2C crossover + rails + SQW→D2 ×3. | Stages 2–3 |
| **Sun 14 Jun** | Stage 3 verification ×3: `02_I2C_Scanner` (CORE PASS), `03_RTC_Test` (4× PASS), `04_EEprom_Test` (0 mismatches), coin-cell-only clock retention. Re-flow anything marginal NOW. Stage 4 RGB LED + Stage 5 NTC/LDR circuit ×3. | Stages 3–5 |
| **Mon 15 Jun** | **GATE A: ≥2 units fully pass electrical QC.** If not, diagnose tonight — there are 7 build days left. Stage 6 calibration ×3: `05_VccCal` constant, `06_SleepCurrent` (≤2 µA). Flash BURN-IN profile, set clock + constant, **1-minute interval**, START. Overnight shakedown (8.5 h fills the EEPROM). | Stage 6 |
| **Tue 16 Jun** | Download shakedown data ×3. Check: no gaps, battery flat, NTC/LDR responding. Then re-flash, **15-min interval**, start the real 48–72 h burn-in on coin cell. | Stage 6 |
| **Wed 17 Jun** | Burn-in running — hands off. Prep sensor modules: bench-test every BMP280 & BH1750 on a spare ProMini (`02` + `07`) so only known-good sensors get soldered. | Stage 7 prep |
| **Thu 18 Jun** | Burn-in running. Buffer day for any rework queued from earlier checks. | — |
| **Fri 19 Jun** | **GATE B: download burn-in from all 3; ≥1 unit (target 2) must be clean.** Stage 7 on the two best units: solder BMP280 + BH1750 to the I2C bus, `02_I2C_Scanner` sees 0x76 + 0x23, `07_SensorTest` wiggle test PASS. | Stage 7 |
| **Sat 20 Jun** | Flash DEPLOY profile on both; startup must list 8 bytes/rec + 5 channels. Short live test (1-min interval, ~1 h), download, confirm all channels sane. | Stage 7 |
| **Sun 21 Jun** | Rework buffer: fix anything Sat exposed. Hot-glue coin cells. Final visual QC under magnification, flux cleanup. | — |
| **Mon 22 Jun** | **GATE C — LAST SOLDER DAY.** Any joint you're unsure about gets re-flowed this morning. Evening: fresh CR2032s, flash/menu sequence (clock `[2]`, constant `[10]`, **15-min interval** `[3]`, START `[6]`) on both units. Final burn-in begins tonight. | Stage 6 seq. |
...
- No falcon-tube housing, no rail-buffer caps, no 32k EEPROM, no RTC aging calibration —
  none are needed for a working prototype.
